...
// 水位。地形(水域内を水底へ沈める)と水面メッシュの上下関係を成立させる共有値。
// 水面は水底より上、かつ地形が水底まで沈んだ場所でだけ見える
export const WATER_BED_Y = -12;      // 平坦化された水底
export const WATER_SURFACE_Y = -11.6; // 水面メッシュの基準高さ(川ごとに+0.15ずつ嵩上げ)

export const GRID_CELL = 130;   // 空間ハッシュのセルサイズ
export const ROAD_STEP = 20;    // 道路パス上の点の間隔(m)
export const N_CARS = 5000;     // 走行車両の台数
export const CAR_MAJOR_TRAFFIC = 3; // 大通りの車密度の重み(他の道路=1)。総台数は不変なので他は薄まる
