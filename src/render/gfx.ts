...
export interface Gfx {
  ...
  quality: QualityMode;     // 現在の画質プリセット(applyQualityが更新。resizeGfxの解像度上限が参照)
...
export function createGfx(canvas: HTMLCanvasElement): Gfx {
  ...
  const gfx: Gfx = {
    canvas, quality: DEFAULT_QUALITY, renderer, scene, camera, hemi, sun, sunShadow,
...
export function resizeGfx(gfx: Gfx): void {
  // devicePixelRatioはDPIの違うモニターへの移動やブラウザズームで変わる。
  // resizeイベントのたびに再評価しないと、古い比率のままぼやけた描画が続く
  applyPixelRatio(gfx);
...
function applyPixelRatio(gfx: Gfx): void {
  const ratio = Math.min(window.devicePixelRatio, QUALITY[gfx.quality].ratio);
