...
import { flushRange, HIDDEN_MAT, setInstanceAt } from '../render/instanced';
...
export class DebrisSystem {
  ...
  update(dt: number, terrain: Terrain): void {
    ...
    let lo = DEBRIS_N, hi = -1;   // 今フレームに書いたスロット範囲(GPU転送の範囲指定用)
    this.pool.sweep(d => {
      if (d.slot < lo) lo = d.slot;
      if (d.slot > hi) hi = d.slot;
      d.age += dt;
      if (d.age >= d.life) {
    ...
    if (hi >= lo) flushRange(this.mesh.instanceMatrix, lo, hi, 16);
  }
}
